...
Now open `.env` and set your `DISCORD_TOKEN`:
```env
DISCORD_TOKEN=your_discord_bot_token_here
DATABASE_URL=postgresql+asyncpg://bot_user:bot_password_123@db:5432/pierceai_bot_db
```
...
1. **Enable bot access and configure permissions for a channel:**
   ```text
   /config channel_settings channel:#channel-name allow_read:True allow_write:True cooldown:3.0
   ```
...
### 4. Trigger
Type the trigger word in any authorized channel to generate a meme:
```text
g.p
```
